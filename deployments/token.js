const { bsv, buildContractClass, getPreimage, toHex, num2bin, Bytes, signTx, PubKey, SigHashPreimage, Sig } = require("scryptlib");
const { DataLen, loadDesc, createLockingTx, sendTx, reverseEndian, showError } = require("../helper");
  ...
  try {
    ...
    let splitTxid, lockingScript0, lockingScript1;
    {
      ...
      const unlockingScript = token
        .split(new Sig(toHex(sig1)), new PubKey(toHex(publicKey2)), 70, outputAmount, new PubKey(toHex(publicKey3)), 30, outputAmount, new SigHashPreimage(toHex(preimage)))
        .toScript();
    ...
    // merge one UTXO with 70 tokens and one with 30 into a single UTXO of 100 tokens
    {
      ...
      // input 0
      {
        ...
        const unlockingScript = token
          .merge(new Sig(toHex(sig2)), new PubKey(toHex(publicKey1)), new Bytes(prevouts), 30, outputAmount, new SigHashPreimage(toHex(preimage)))
          .toScript();
        tx.inputs[0].setScript(unlockingScript);
      }
      ...
      // input 1
      {
        ...
        const unlockingScript = token
          .merge(new Sig(toHex(sig3)), new PubKey(toHex(publicKey1)), new Bytes(prevouts), 70, outputAmount, new SigHashPreimage(toHex(preimage)))
          .toScript();
