...
describe("Test sCrypt contract NFT In Javascript", () => {
  ...
  before(() => {
    ...
    token = new Token(
      ...
      new Bytes("85d29f6807cee298dc66a1ed080b7048abcb8dcb2e136715f98467637b3e91ce" + num2bin(0, DataLen4)),
      new Bytes(num2bin(0, DataLen4))
    );
    ...
    makeTxGenesis = (prevTxId, pkhTheIssuer, lastTokenId) => {
      ...
      const newLockingScript = [lockingScriptCodePart, toHex(pkhTheIssuer) + num2bin(lastTokenId, DataLen8) + dataTypeIssue].join(" ");
    ...
    makeTxIssue = (prevIssueTxId, pkhNewReceiver, pkhTheIssuer, pkChange, lastTokenId, nextTokenId) => {
      ...
      // output 0
      const newLockingScript0 = [lockingScriptCodePart, toHex(pkhTheIssuer) + num2bin(nextTokenId, DataLen8) + dataTypeIssue].join(" ");
      txnew.addOutput(
        new bsv.Transaction.Output({
      ...
      // output 1
      const newLockingScript1 = [lockingScriptCodePart, toHex(pkhNewReceiver) + num2bin(nextTokenId, DataLen8) + dataTypeTransfer].join(" ");
      txnew.addOutput(
        new bsv.Transaction.Output({
    ...
    makeTxTransfer = (prevTransferTxId, pkhOwner, pkhNewReceiver, pkChange, lastTokenId, transferTokenId) => {
      ...
      // input 0
      // prev output 1
      const prevTransferLockingScript1 = [lockingScriptCodePart, toHex(pkhOwner) + num2bin(lastTokenId, DataLen8) + dataTypeTransfer].join(" ");
      ...
      // output 0
      const newLockingScript0 = [lockingScriptCodePart, toHex(pkhNewReceiver) + num2bin(transferTokenId, DataLen8) + dataTypeTransfer].join(" ");
      txnew.addOutput(
        new bsv.Transaction.Output({
          script: bsv.Script.fromASM(newLockingScript0),
          satoshis: 50000000,
  ...
  it("should succeed when one new token is issued", async () => {
    const testIssue = async (privKey, pkhGenesisIssuer, pkhNewReceiver, pkhNewIssuer, nextTokenId, followGenesis) => {
      ...
      // 设置校验环境
      token.setDataPart(toHex(pkhGenesisIssuer) + num2bin(currTokenId, DataLen8) + dataTypeIssue);
  ...
  it("should succeed when a token is transferred", async () => {
    const testTransfer = async (privKeyIssue, privKeyTranse, pkhGenesisIssuer, pkhNewIssuer, pkhOwner1, pkhOwner2, pkOwner1, transTokenId) => {
      ...
      token.setDataPart(toHex(pkhOwner1) + num2bin(currTokenId + 1, DataLen8) + dataTypeTransfer);
  ...
  it("should success when receiver burn the token", async () => {
    const testBurn = async (privKeyTranse, pkhGenesisIssuer, pkhNewIssuer, pkhOwner, pkOwner, transferTokenId) => {
      ...
      token.setDataPart(toHex(pkhOwner) + num2bin(transferTokenId, DataLen8) + dataTypeTransfer);
      token.txContext = {
        tx: txTransferBurn,
        inputIndex,
