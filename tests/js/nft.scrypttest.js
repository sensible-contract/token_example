...
const { inputIndex, inputSatoshis, tx, compileContract, DataLen, dummyTxId, satoTxSigUTXOSpendBy, satoTxSigUTXO } = require("../../helper");
...
const sighashType = Signature.SIGHASH_ANYONECANPAY | Signature.SIGHASH_ALL | Signature.SIGHASH_FORKID
   ...
   it("should succeed when a token is transferred", async () => {
      ...
      */
      ...
      let verifyData = await testTransfer(privateKeyIssuer, privateKeyReceiver1, publicKeyHashIssuer, publicKeyHashIssuer, publicKeyHashReceiver1, publicKeyHashIssuer, publicKeyReceiver1, currTokenId + 1);
      let result = verifyData.verify();
      expect(result.success, result.error).to.be.true;

      // unauthorized key
      verifyData = await testTransfer(privateKeyIssuer, privateKeyIssuer, publicKeyHashIssuer, publicKeyHashIssuer, publicKeyHashReceiver1, publicKeyHashIssuer, publicKeyReceiver1, currTokenId + 1);
      result = verifyData.verify();
      expect(result.success, result.error).to.be.false;

      // token ID must not change
      verifyData = await testTransfer(privateKeyIssuer, privateKeyReceiver1, publicKeyHashIssuer, publicKeyHashIssuer, publicKeyHashReceiver1, publicKeyHashIssuer, publicKeyReceiver1, currTokenId + 2);
      result = verifyData.verify();
      expect(result.success, result.error).to.be.false;
   });
